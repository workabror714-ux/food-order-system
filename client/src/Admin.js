...
// sifat: 0.0-1.0, maxWidth: maksimal kenglik
...
export default function Admin() {
  ...
  const resetForm = () => {
    ...
    setImage(null); setImagePreview(null); setEditId(null);
  };

  // Rasm tanlash + compress
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      console.log(`Rasm: ${sizeBefore}KB → ${sizeAfter}KB`);
    } catch {
      setImage(file);
    } finally {
  ...
  const confirmDeleteCategory = async () => {
    ...
    if (catFoods.length > 0) {
      if (deleteCatAction === "delete") {
        if (!window.confirm(`"${name}" kategoriyasidagi ${catFoods.length} ta taom ham o'chiriladi.`)) return;
  ...
  const handleEdit = (food) => {
    ...
    setImage(null); setImagePreview(food.image?.startsWith("http") ? food.image : `${API}${food.image}`);
  ...
  const handleBannerSave = async () => {
    ...
    try {
      ...
      if (bannerMedia) {
        // Banner rasmini ham compress qilamiz
        const compressed = await compressImage(bannerMedia, 1600, 0.85);
              ...
              <form onSubmit={handleFoodSubmit} className="food-form">
                <div className="form-grid">
                  <div className="input-group">
                    ...
                    <input type="text" placeholder="Masalan: Shashlik" value={title}
                        ...
                        {categories.map(cat => (
                          <div key={cat} className="cat-chip-wrap">
                            ...
                            <button type="button" className="cat-delete-btn"
                              onClick={e => { e.stopPropagation(); openDeleteCatModal(cat); }}>✕</button>
                          </div>
                        ))}
                  ...
                  <div className="input-group">
                    ...
                    <input type="file" accept="image/*"
                      onChange={handleImageChange} required={!editId} />
                    {compressing && (
                      <p style={{ fontSize: "0.82rem", color: "var(--g)", marginTop: 4 }}>
                      ...
                      <div style={{ marginTop: 10 }}>
                        <img src={imagePreview} alt="preview"
                          style={{ width: "100%", maxHeight: 180, objectFit: "cover",
                            borderRadius: 12, border: "2px solid var(--g3)" }} />
                        {image && (
                          <p style={{ fontSize: "0.78rem", color: "var(--gray)", marginTop: 4 }}>
                            📦 Hajm: {(image.size / 1024).toFixed(0)} KB
                          </p>
                        )}
                      </div>
                    )}
                ...
                <div className="form-actions">
                  <button type="submit" className="btn-primary" disabled={foodLoading || compressing}>
                    {foodLoading ? "Saqlanmoqda..." : compressing ? "Rasm optimallashmoqda..." : editId ? "💾 Saqlash" : "➕ Qo'shish"}
                  </button>
              ...
              <div className="food-admin-grid">
                {foods.map(food => (
                  <div key={food._id} className="food-admin-card" onClick={() => setSelectedFood(food)}>
                    ...
                    />
                    <div style={{ display: "none", height: 150, background: "#e8f5ee",
          ...
          <div className="admin-section">
            ...
            <div className="order-filter-bar">
              {["all", "new", "preparing", "delivered", "cancelled"].map(s => (
                <button key={s} className={`filter-btn ${orderFilter === s ? "active" : ""}`}
                  onClick={() => setOrderFilter(s)}>
                  {s === "all" ? "Barchasi" : statusLabel[s]}
                  {s === "new" && newOrderCount > 0 && (
                    <span style={{ background: "#ef4444", color: "#fff", borderRadius: "50%",
                      padding: "1px 6px", fontSize: 11, marginLeft: 6 }}>{newOrderCount}</span>
                  )}
                </button>
              ))}
            </div>
            ...
            <div className="food-form" style={{ marginTop: 20 }}>
              ...
              {banner.mediaType !== "none" && (
                <div className="input-group" style={{ marginTop: 12 }}>
                  <label>{banner.mediaType === "image" ? "Rasm" : "Video"} yuklash (yoki URL)</label>
                  <input type="file" ref={bannerFileRef}
                    accept={banner.mediaType === "image" ? "image/*" : "video/*"}
                    onChange={e => setBannerMedia(e.target.files[0])} />
                  <input type="text" placeholder="Yoki to'g'ridan-to'g'ri URL..."
              ...
              <div style={{ marginTop: 20 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 12 }}>
                  {banner.events?.map(ev => (
                    <span key={ev.id} style={{
                      ...
                      background: "#e8f5ee", padding: "5px 12px", borderRadius: 20, fontSize: "0.85rem", color: "#0d4a28"
                    }}>
                      {ev.emoji} {ev.label}
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <input type="text" placeholder="🔥" value={newEvent.emoji}
                    ...
                    style={{ width: 70 }} />
                  <input type="text" placeholder="Chegirma 20%, Trendda..."
          ...
          <div className="modal-card cat-delete-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <h2 className="modal-title" style={{ fontSize: "1.2rem" }}>
                "<span style={{ color: "var(--g)" }}>{deleteCatModal.name}</span>" o'chirish
              </h2>
