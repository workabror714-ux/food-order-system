...
export default function Admin() {
  ...
  useEffect(() => { fetchFoods(); fetchAdmins(); }, []);
    ...
    <div className="admin-root">
      ...
      <div className="admin-tabs">
        ...
        <button className={`admin-tab ${tab === "orders" ? "active" : ""}`} onClick={() => setTab("orders")}>
          📋 Buyurtmalar
          {newOrderCount > 0 && <span className="tab-badge">{newOrderCount}</span>}
        </button>
          ...
          <>
            ...
            <div className="admin-section">
              ...
              <p style={{ fontSize: "0.85rem", color: "#888", marginBottom: 16 }}>
                💡 Taom kartasiga bosib to'liq ma'lumotni ko'ring
              </p>
              ...
              {admins.map((adm) => (
                <div key={adm._id} className="admin-row">
                  ...
                  {adm.username !== savedUser.username && (
                    <button className="btn-delete" style={{ marginLeft: "auto", flex: "none" }} onClick={() => handleDeleteAdmin(adm._id)}>
                      🗑 O'chirish
                    </button>
                  )}
                </div>
              ...
              {deleteCatModal.foods.length === 0 ? (
                ...
                <>
                  ...
                  <div className="cat-del-food-list">
                    {deleteCatModal.foods.map((f) => (
                      <span key={f._id} className="order-item-chip">{f.title}</span>
                    ))}
                  </div>
