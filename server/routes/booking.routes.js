...
// Joy bron qilish arizasi — buyurtmalar kanaliga tushadi, mas'ul mijoz bilan bog'lanadi
router.post("/api/booking", rateLimit({ windowMs: 60000, max: 5 }), async (req, res) => {
  try {
    ...
    const targetChat = TG_CHANNEL || TG_STAFF || TG_CHAT;
    if (!targetChat) {
      console.error("[booking] TELEGRAM_CHANNEL_ID / TELEGRAM_STAFF_CHAT_ID / TELEGRAM_CHAT_ID sozlanmagan");
      return res.status(503).json({ message: "Bron kanaliga ulanish sozlanmagan. Restoranga qo'ng'iroq qiling." });
    }
    ...
    if (!result?.ok) {
      console.error("[booking] Telegram xatosi:", result);
      return res.status(502).json({ message: "Bron arizasini yuborib bo'lmadi. Restoranga qo'ng'iroq qiling." });
    }
