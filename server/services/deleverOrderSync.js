...
const syncOrderToDelever = async (
  orderOrId,
  {
    force = false,
  } = {}
  ...
  try {
    ...
    console.error(
      `Delever order sync xato (${claimed._id}):`,
      error.message
    );
...
const retryPendingDeleverOrders =
  async ({
    limit,
  } = {}) => {
    const config =
...
const refreshDeleverOrderStatus =
  async (orderOrId) => {
    const orderId =
      String(
        orderOrId?._id ||
    ...
    if (
      !order.deleverOrderId
    ) {
      return {
        skipped: true,
        reason:
          "delever_order_id_missing",
      };
    }
